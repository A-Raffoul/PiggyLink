...
Detail moved out of the README to keep it short. Everything here describes the
same app as the demo.

## Turn flow

Each turn has a spoken line and a hidden message (up to 64 UTF-8 bytes).
...
- Included example speech or custom WAV cover audio (looped when a message needs
  longer), with messages up to 64 UTF-8 bytes
- ggwave Ultrasound Fastest (falls back to Fast, then Normal) with manually
  matched 15, 16, 17, or 18 kHz presets
- Adjustable carrier level from -30 to -12 dB relative to the speech in the
  overlay window
